...
void CTRCParser::parse(const char *dir,HTRCHeader *pHead,double *mat)
{
	FILE *f = fopen(dir,"r");
	fscanf(f,"PathFileType\t%d\t(%c/%c/%c)\t%s\n",\
		&pHead->m_pathFileType,\
	...
	for(int i = 0;i < pHead->m_numFrames;i++)
	{
		...
		{
                        int index = INDEX(i,j*3,pHead->m_numMarkers*3);
                        fscanf(f,"%lf%lf%lf",&mat[index+0],&mat[index+1],&mat[index+2]);
		}
                fscanf(f,"\n");
	}
	fclose(f); f = NULL;
...
void CTRCParser::resotre(const char *dir,const HTRCHeader *pHead,const double *mat)
{
	FILE *f = fopen(dir,"w");
	...
	for(int i = 0;i < pHead->m_numFrames;i++)
	{
		...
		{
                        int index = INDEX(i,j*3,pHead->m_numMarkers*3);
                        fprintf(f,"%0.6lf\t%0.6lf\t%0.6lf\t",mat[index+0],mat[index+1],mat[index+2]);
		}
		fprintf(f,"\n");
	}
	fclose(f); f = NULL;
